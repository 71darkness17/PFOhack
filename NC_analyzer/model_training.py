...
def build_neural_network(input_dim):
    ...
    # Создаем выходной слой с вероятностью прекращения использования услуг компании
...
def train_model(data):
    ...
    features = data.drop(columns = ['client_id', 'target', 'regions'])
    target = data['target']
    # Разделяем данные на обучающую и тестовую выборки (60% - обучающие данные, 40% - тестовые данные; регулируется переменной test_size)
...
def main():
    if not os.path.isfile("training_input//dataset.csv"):
        paths = []
        ...
        paths.extend(list(glob.glob("meta//target.xlsx")))
        paths.extend(list(glob.glob("meta//Привязка ID.xlsx")))
        create_dataset(paths, 'training_input//dataset.csv', 1)
    # Загружаем входные данные
    data = load_all_data('training_input')
