...
def sorting(clients: list, df: DataFrame):
    ...
    for id in df['client_id']:
        ...
        capital_procent = (avg - current) / avg * 100
        common_procent = (0.001 * capital_procent + cur_client[1][0]) / 2
        sorted_clients.append((id, common_procent.item(), cur_client[1][1], cur_client[1][2]))
    sorted_clients.sort(key = lambda x: (x[1], x[0]), reverse = True)
