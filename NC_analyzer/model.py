...
def main():
    if not os.path.isfile("test_input//dataset.csv"):
        paths = []
        paths.extend(list(glob.glob("meta/rzd/Выгрузка_маркетинговые списки/*")))
        paths.extend(list(glob.glob("meta/rzd/Выгрузки_интересы+обращения+объёмы перевозок/*")))
        paths.extend(list(glob.glob("meta/rzd/Привязка ID.xlsx")))
